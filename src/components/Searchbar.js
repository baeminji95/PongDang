...
export default function SearchBar() {
  ...
  const [forecast, setForecasth] = useState(null);
  const [wt, setWt] = useState(null);
  const [temperatures, setTemperatures] = useState(null);
  const [vilageFcst, setVilageFcst] = useState(null);
  const [alldayWt, setAlldayWt] = useState(null);
  const [alldayWh, setAlldayWh] = useState(null);
        ...
        >
          ...
          <div className="w-[100%] flex justify-center">
            ...
            <div className="w-[10rem] xl:w-[12rem] xl:mx-8 flex flex-col justify-between my-3 ml-5 xl:pl-3">
              ...
              ) : forecast.response.body.items.item[8].fcstValue === "1" || "2" || "5" ? (
                <p className="text-[4.5rem] relative xl:text-[8rem]">
                  🌧
